(function($) {
    ...
    if ($(".comparison-slider__wrapper")[0]) {
        let compSlider = $(".comparison-slider__wrapper");

        compSlider.each(function() {
            let compSliderWidth = $(this).width() + "px";
            $(this).find(".comparison-slider__resize img").css({
                width: compSliderWidth
            });
            drags($(this).find(".comparison-slider__divider"), $(this).find(".comparison-slider__resize"), $(this));
        });

        $(window).on("resize", function() {
            let compSliderWidth = compSlider.width() + "px";
            compSlider.find(".comparison-slider__resize img").css({
                width: compSliderWidth
            });
        });
    }
    ...
    // Date Picker
    if ($(".ploming-datepicker").length) {
        $(".ploming-datepicker").each(function() {
            $(this).datepicker();
        });
    }
    ...
    //accordion
    if ($(".ploming-accordion").length) {
        var accordionGrp = $(".ploming-accordion");
                ...
                $(this)
                    ...
                    .on("click", function() {
                        if ($(this).parent().hasClass("active") === false) {
                            $(".ploming-accordion." + accordionName)
                                .find(".accordion")
                                .removeClass("active");
                            $(".ploming-accordion." + accordionName)
                                .find(".accordion")
    ...
    function thmOwlInit() {
        // owl slider
        let plomingowlCarousel = $(".ploming-owl__carousel");
        if (plomingowlCarousel.length) {
            plomingowlCarousel.each(function() {
                let elm = $(this);
        ...
        let plomingowlCarouselNav = $(".ploming-owl__carousel--custom-nav");
        if (plomingowlCarouselNav.length) {
            plomingowlCarouselNav.each(function() {
                let elm = $(this);
        ...
        let plomingowlCarouselWithCounter = $(
            ".ploming-owl__carousel--with-counter"
        );
        if (plomingowlCarouselWithCounter.length) {
            plomingowlCarouselWithCounter.each(function() {
                let elm = $(this);
                let options = elm.data("owl-options");
                ...
                elm
                    .on("initialized.owl.carousel", function(event) {
                        ...
                        elm.append(
                            '<div class="ploming-owl__carousel__counter"><span class="ploming-owl__carousel__counter__total"></span> <span class="ploming-owl__carousel__counter__current"></span></div>'
                        );
                        elm
                            .find(".ploming-owl__carousel__counter__current")
                            .text(
                                addLeadingZero(carousel.relative(carousel.current()) + 1, 2)
                            );
                        elm
                            .find(".ploming-owl__carousel__counter__total")
    ...
    //Strech Column
    function ploming_stretch() {
        var i = $(window).width();
        $(".row .ploming-stretch-element-inside-column").each(function() {
            var $this = $(this),
